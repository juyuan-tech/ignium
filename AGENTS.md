...
2. **初始化顺序** kernel_main:irq_disable → sanitize_csr → uart::init
   → init_traps → enable_timer → set_level → FDT 解析 + board::init_from_fdt
   → uart::reinit → cpu::init_from_fdt → mem::init(&fdt_params)/mmu::init
   /heap::init/sched::init →(自检)→ irq_enable。
