//! 分级日志系统。
//!
//! 格式:`[{tick:06}] [{级别}] 消息`,tick 由 M1 定时器中断递增(10ms
//! 节拍),用于把日志与中断/时间轴对应。
//!
//! # 级别语义
...
//! # 并发/重入约束(团队注意)
//! 当前**无锁**,契约如下:
//! - **中断/ISR 上下文禁止日志**(定时器 ISR 只做 tick_up +
//!   sched::on_tick(帧复制/抢占决策),零输出)—— 无锁写会与主
//!   上下文交错,破坏诊断可读性。
//! - **同步异常路径(trap_handler)可输出**(异常即停机,输出为终局;
//!   嵌套风险已由陷阱栈吸收)。M1 调度已就绪,日志仍保持无锁
//!   (ISR 禁止日志约束);M2 若需 ISR/中断日志,升级为锁或 ISR 缓冲。
//!
//! # 性能
//! 级别过滤发生在 `format_args!` 展开之前,被过滤的调用**零格式化
//! 开销**。注意:宏**参数表达式**仍会被立即求值(如 `info!("{}", f())`
//! 即使被过滤也会调用 `f()`);调用方不要把昂贵表达式直接放进参数
//! (pro 审计 #6)。
