...
pub extern "C" fn _start(argc: usize, _argv: usize) -> ! {
    ...
    let fd = open_reply[2];
    // 4) 内联负面用例(任一失败 → 提前 exit,marker 不到)。
    //    a) EOF:空文件 offset=0 read → 0 字节(非错误)。
    if fs_request(OP_FS_READ, &[fd, 0, 16, 0], |r| r[2] == 0 && r[3] == 0).is_err() {
        sys_exit();
    }
    ...
    if fs_request(OP_FS_READ, &[99, 0, 1, 0], |r| r[2] == USER_ERR_EBADF).is_err() {
        sys_exit();
    }
    //    d) 越界写(off=4095, len=2 → off+len=4097 > 4096)→ EINVAL。
    if fs_request(OP_FS_WRITE, &[fd, 4095, 2, 0], |r| r[2] == USER_ERR_EINVAL).is_err() {
        sys_exit();
    }
    ...
    if fs_request(OP_FS_READ, &[fd, 0, PAYLOAD.len(), 0], |r| r[2] == 0).is_err() {
        sys_exit();
    }
    for (i, b) in PAYLOAD.iter().enumerate() {
    ...
    if fs_request(OP_FS_CLOSE, &[fd, 0, 0, 0], |r| r[2] == 0).is_err() {
        sys_exit();
    }
    if fs_request(OP_FS_UNLINK, &[fd, 0, 0, 0], |r| r[2] == 0).is_err() {
        sys_exit();
    }
