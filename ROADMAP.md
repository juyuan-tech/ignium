...
## 阶段 1.5:稳定化与真机准备(M1.5)

承接 DEFERRED 中触发条件已到的各项。产出:**真机可引导的第一版内核**。

| 任务 | 来源 | 产出/验收 |
|---|---|---|
| FDT 解析:RAM 大小 / UART 基址与时钟 / 定时器频率 / 保留区实际大小 | D5 | board.rs 由编译期常量改为引导期数据(回退默认值) |
| 页权限拆分:代码 RX / 数据 RW / MMIO RW(去除整体 RWX) | D2 | 越权访问(写代码段)触发页故障而非静默成功 |
| 栈守护页:boot/trap 栈下方不可映射页(MMU 已就绪) | D4 | 栈溢出 → 页故障停机(而非静默损坏) |
| RVA23 P1:编译目标 Zba/Zbb/Zbs+Zicond + CI `-cpu max` 矩阵 + ISA 探测 | D16 | 双 CPU 基线 CI 全绿;启动输出 ISA 能力表 |
| 页表接口补全:`arch_mmu_unmap` / TLB flush 封装 | 前瞻(M2 前置) | 为 M2 用户态映射/解除映射打底 |
| 压力自检:调度/互斥高频竞争(万级锁争用、多线程交错) | 审计 13 轮建议 | 10 万次以上锁操作无丢失/重复 |
