...
> **当前进度:T0 ✓ / T1 ✓(含每进程地址空间与 D20)/ T2a ✓**(同步 IPC
> 核心:寄存器消息 + 阻塞配对 + 简化能力表 + D22 woken 抢占)→ 下一步
> **T2b**(优先级继承/压力测试)、T3(共享内存/revoke)。
...
| **IPC 设计**:同步 IPC + 注册发送 + 阻塞/唤醒 + 优先级继承(依赖 D22 woken 抢占,见 M2-DESIGN §5.1) | A→B 消息往返正确 | ✅ **T2a ✓**(同步 IPC 核心:寄存器消息 + 阻塞配对 + 简化能力表 + D22 woken 抢占,引导期往返测试通过);**T2b 待办**(优先级继承 + 压力测试) |
| IPC 性能:寄存器小消息 + 共享内存大消息 | 延迟可测并记录 | 寄存器小消息已落地(T2a);共享内存大消息归 T3 |
| 能力模型简化版:未授权 IPC 被拒 | 拒绝测试用例 | ✅ T2a:未授权 cap → `-EACCES` 拒绝测试通过 |
| **用户态异常恢复**:用户进程故障 → 杀进程而非整机停机(D12,需 per-hart 应急栈) | 用户进程触发 page fault/非法指令,系统存活、其余进程不受影响 |
| 多核前置:per-hart 陷阱栈(D7)、控制台锁(D9)、副核唤醒(D8)、多核调度器(D19) | 4 核 QEMU 上所有 hart 进入 idle,调度器可分配线程到各核 |
