...
> 前期准备(F41 起):`docs/M2-DESIGN.md` 设计 + T0 地基(用户页清零 D10、
> 页表用户映射契约、线程 TCB 槽复用、D22 woken 抢占)。设计先行,再按
...
| 多核前置:per-hart 陷阱栈(D7)、控制台锁(D9)、副核唤醒(D8) | 4 核 QEMU 上所有 hart 进入 idle,调度器可分配线程到各核 |
